...
class robot:
    ...
    def __init__(self, robot_name, ros_namespace = '/dvrk/'):
        ...
        # Brijen's masters states

        if robot_name == "PSM1":
            self.master_name = "MTMR"
        else:
            self.master_name = "MTML"
        self.__master_position_cartesian_current = None
        self.master_joint_current = None
    ...
    def __master_position_cartesian_current_callback(self, data):
        self.__master_position_cartesian_current = [data.position.x, data.position.y, data.position.z, data.orientation.x, data.orientation.y, data.orientation.z, data.orientation.w]

    def __master_joint_current_callback(self, data):
        self.__master_joint_current = data.position

    def get_master_position_cartesian_current(self):
        return self.__master_position_cartesian_current

    def get_master_joint_current(self):
        return self.__master_joint_current
